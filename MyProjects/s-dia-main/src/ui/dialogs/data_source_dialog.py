...
from ui.common.file_utils import get_schema, read_file
...
from ..database import add_datasource, write_datasource
...
class DataSourceDialog(QDialog):
    ...
    def _after_show_setup(self):
        try:
            if self.data:
                has_header = "있음" if self.data.get('has_header', False) == True else "없음"
                ...
                self.has_header_input.setCurrentText(has_header)
                ...
                if schemas:
                    ...
                    # 원본 데이터에서 샘플 값 추출
                    file_schemas, first_rows = read_file(data_path, self.data.get('data_type') == 'csv', 0, "UTF-8", ",", 3)
    ...
    def setup_schema_table(self, data_list):
        ...
        self.schema_table.setColumnWidth(1, 200)  # 컬럼명
    ...
    def accept(self):
        ...
        try:
            ...
            # add_datasource(self.project_data.get('uuid', ''), self.get_data())
            # # write_datasource(self.project_data.get('uuid', ''))
    ...
    def get_data(self):
        ...
        return {
            ...
            'has_header': True if self.has_header_input.currentText() == '있음' else False,
            'schemas': schemas
        }
            ...
            if file_path:
                ...
                try:
                    ...
                    if self.is_csv:
                        ...
                        # 파일의 인코딩 감지 (최적화: 샘플만 읽기)
                        try:
                            # 파일 크기 확인
                            file_size = os.path.getsize(file_path)
                            
                            # 큰 파일의 경우 샘플만 읽어서 인코딩 감지
                            sample_size = min(1024 * 1024, file_size)  # 최대 1MB 샘플
                            ...
                            with open(file_path, 'rb') as file:
                                ...
                                logging.debug(f"감지된 파일 인코딩: {detected_encoding} (신뢰도: {confidence:.2%})")
                            ...
                            # 인코딩 감지 실패 시 기본값 사용
                            logging.debug(f"인코딩 감지 실패, 기본값 사용: {e}")
                            self.charset_input.setCurrentText('UTF-8')
                    else:
                        self.csv_container.hide()
    ...
    def load_data(self):
        ...
        try:
            ...
            if not file_path:
                return 
            self.is_csv = True if file_path.lower().endswith('.csv') else False
            header_line = 0 if self.is_csv and self.has_header_input.currentText() == '있음' else None
            sep = ','
            ...
            if os.path.exists(file_path):
                file_schemas, first_rows = read_file(file_path, self.is_csv, header_line, self.charset_input.currentText(), sep, 3)
            ...
            self.message_text.setText(f'{error_msg.replace("\n", "")}')
            self.message_container.show()
