...
class FlowDialog(QDialog):
    ...
    def accept(self):
        ...
        try:
            ...
            if not name:
                QMessageBox.warning(self, "입력 오류", "흐름 명을 입력하세요.")
                self.name_input.setFocus()
                return

            if not selected_data_source:
                QMessageBox.warning(self, "입력 오류", "입력 데이터를 선택하세요.")
            ...
            # 프로젝트 UUID 디렉토리
            project_dir = os.path.join(data_dir, self.parent.project_data.get('uuid', ''))
            flow_path = os.path.join(project_dir, 'flow.json')
            ...
            for flow in flows:
                if flow.get('name', '').strip() == name:
                    ...
                    if self.flow and flow.get('uuid') == self.flow.get('uuid'):
                        continue
                    QMessageBox.warning(self, "입력 오류", f"'{name}' 흐름명이 이미 존재합니다.")
                    self.name_input.setFocus()
                    return
            ...
            if not data_source:
                QMessageBox.warning(self, "입력 오류", "선택한 데이터 소스를 찾을 수 없습니다.")
                return
            ...
            flow_data = {
                ...
                'nodes': self.flow.get('nodes', []) if self.flow else [],  # 수정 시 기존 노드 유지
                'edges': self.flow.get('edges', []) if self.flow else []   # 수정 시 기존 엣지 유지
            }

            # 플로우 목록에 추가 또는 업데이트
            if self.flow:  # 수정인 경우
                for idx, f in enumerate(flows):
                    if f.get('uuid') == self.flow.get('uuid'):
                        flows[idx] = flow_data
                        break
            else:  # 새로 추가하는 경우
            ...
            QMessageBox.critical(self, "오류", f"흐름 저장 중 오류가 발생했습니다.\n{str(e)}")
