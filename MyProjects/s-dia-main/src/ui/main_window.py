...
class MainWindow(QMainWindow):
    def __init__(self, user_info):
        ...
        if not self.ui:
            ...
            QMessageBox.critical(self, "오류", "UI 파일을 로드할 수 없습니다.")
            return
    ...
    def on_project_delete(self, card):
        ...
        # 공통 확인 다이얼로그 사용
        if ConfirmDialog.show(self, "프로젝트 삭제", 
                           f"'{card.project_data.get('name', '')}' 프로젝트를 삭제하시겠습니까?\n이 작업은 되돌릴 수 없습니다."):
            try:
                # 프로젝트 목록에서 제거
                projects = self.load_projects_from_json()
                projects = [p for p in projects if p.get('name') != card.project_data.get('name', '')]
                ...
                QMessageBox.critical(self, "오류", "프로젝트 삭제 중 오류가 발생했습니다.")
    ...
    def reorder_cards(self, source_card, target_card):
        ...
        try:
            ...
            # 프로젝트 데이터 순서도 변경
            projects = self.load_projects_from_json()
            projects.insert(target_idx, projects.pop(source_idx))
