...
class ProjectPanel(QWidget):
    ...
    def delete_flow(self, flow):
        ...
        if reply == QMessageBox.Yes:
            ...
            flows = self._get_current_flows()
            # uuid로 삭제
            flows = [f for f in flows if f.get('uuid') != flow.get('uuid')]
            # 저장
            if os.name == 'nt':
                data_dir = os.path.join(os.getenv('LOCALAPPDATA', ''), 'UPSDATA', 'S-DIA', 'projects')
            else:
    ...
    def _get_current_flows(self):
        """현재 프로젝트의 흐름 목록을 반환합니다."""
        if os.name == 'nt':
            data_dir = os.path.join(os.getenv('LOCALAPPDATA', ''), 'UPSDATA', 'S-DIA', 'projects')
        else:
            data_dir = os.path.expanduser('~/.upsdata/s-dia/projects')
