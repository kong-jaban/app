...
class UserInfoDialog(QDialog):
    ...
    def validate_and_save(self):
        ...
        if not self.id_input.text().strip():
            MessageDialog.warning(self, "경고", "사용자 ID를 입력하세요.")
            return False
        if not self.name_input.text().strip():
            MessageDialog.warning(self, "경고", "사용자명을 입력하세요.")
            return False
        # 비밀번호 검증
        ...
        if current_password != confirm_password:
            MessageDialog.warning(self, "경고", "비밀번호가 일치하지 않습니다.")
            return False
        # 비밀번호가 비어있는 경우
        if not current_password:
            MessageDialog.warning(self, "경고", "비밀번호를 입력하세요.")
            return False
        try:
            # 새로운 사용자 정보 구성
            new_user_info = {
            ...
            MessageDialog.information(self, "알림", "사용자 정보가 저장되었습니다.")
            return True
            
        except Exception as e:
            MessageDialog.critical(self, "오류", f"사용자 정보 저장 중 오류가 발생했습니다.\n{str(e)}")
            return False 